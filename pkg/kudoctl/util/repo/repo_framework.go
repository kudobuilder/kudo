...
func (r *FrameworkRepository) DownloadBundle(bundleName string) (FrameworkPackage, error) {
	var fileURL string
	parsedURL, err := url.Parse(r.Config.URL)
	if err != nil {
		return nil, errors.Wrap(err, "parsing config url")
	...
	fvPackage, err := UntarV0Package(resp)

	if err != nil {
		// try to parse package as V1 package
		fvPackageV1, errV1 := UntarV1Package(resp)
		if errV1 != nil {
