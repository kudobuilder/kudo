...
func FilesToResources(files *packages.Files) (*packages.Resources, error) {
	...
	instance := BuildInstanceResource(files.Operator.Name, files.Operator.OperatorVersion, files.Operator.AppVersion)
...
func BuildInstanceResource(operatorName, operatorVersion, appVersion string) *kudoapi.Instance {
	return &kudoapi.Instance{
		...
		Spec: kudoapi.InstanceSpec{
			OperatorVersion: corev1.ObjectReference{
				Name: kudoapi.OperatorVersionName(operatorName, operatorVersion, appVersion),
			},
		},
