...
var (
	upgradeExample = `
		...
		*Note*: should you have a local "flink" folder in the current directory it will take precedence over the remote repository.
		...
		# By default argments are all reused from the previous installation, if you need to modify, use -p
...
func newUpgradeCmd(fs afero.Fs) *cobra.Command {
	...
	upgradeCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			// Prior to command execution we parse and validate passed argments
			var err error
			options.Parameters, err = install.GetParameterMap(parameters)
			if err != nil {
				return errors.WithMessage(err, "could not parse argments")
			}
...
func upgrade(newOv *v1alpha1.OperatorVersion, kc *kudo.Client, options *options) error {
	...
	// Change instance to point to the new OV and optionally update argments
