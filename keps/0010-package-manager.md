...
As more and more developers seed our `kubebuilder/frameworks` repository with incredibly important Frameworks, we need to define how these frameworks will be made available.

This KEP aims to define a framework repository structure. This structure indirectly impacts other KEPs, e.g., [KEP-0003 CLI](https://github.com/kudobuilder/kudo/blob/master/keps/0003-kep-cli.md), [KEP-0007 CLI Generation](https://github.com/kudobuilder/kudo/blob/master/keps/0007-cli-generation.md) or [KEP-0008 Framework Testing](https://github.com/kudobuilder/kudo/blob/master/keps/0008-framework-testing.md).

Overall, this KEP captures how we plan to provide a great user experience for installing Frameworks with KUDO. It focuses on:
...
A repository will be served by a vanilla HTTP(S) server and consists of an index file in YAML and the `tgz` files for each framework. All this arquitecture requires is the ability to serve files using an HTTP server, which is very easy to implement using Google Cloud Storage or S3.

Some caveats of this architecture could for instance be defining a clean interface that won't break when a user attempts to use an older version of our tools to download a Frameworks hosted on a repo using a newer format. As we are still building out this interface with our beta releases this kind of breaking changes are to be expected, but as we get closer to our first stable versions we will have to minimize this kind of changes.
...
The framework versioning schema affects the structure of the repository. Here we agreed that versioning for the `.tgz` files and Frameworks should follow the SemVer convention.
...
We rely on just an HTTP Server, e.g. the out-of-the-box solution that Google Cloud Storage provides, that serves framework `tgz` files and makes them available to users.

The logic for keeping the frameworks in sync should live in the CLI and is not defined on this KEP . That way the HTTP server only has to serve the index and the Package `tgz` files, without having to implement any business logic. 
...
The changes proposed in this KEP are not necessarily backwards compatible and might prevent older versions of the CLI from installing Frameworks.
