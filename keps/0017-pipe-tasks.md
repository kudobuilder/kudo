...
``` 
The key `{{.Pipes.Certificate}}` value will be generated by KUDO to avoid collisions and to make sure that it is not used before the file is generated. In the above example we would generate secret name like `deploy.bootstrap.gencert.pipes.certificate-#hash` to capture what plan/phase/step/task it belongs too as well as the hash of its content. KUDO would use the hash to avoid recreating unchanged files on a plan rerun. We would also use metadata labels ensuring that the secrets are cleaned up when the corresponding Instance is removed.
