...
Allowing to pipe all kind of files between tasks requires a general purpose storage solution (e.g. S3) available in the cluster. While certainly possible, this is not goal of this kep.
...
``` 
They key will be generated by KUDO, avoiding collisions and making sure that it is not used before it is generated.
...
- Provided ContainerSpec is injected into a Pod as an [InitContainer](https://kubernetes.io/docs/concepts/workloads/pods/init-containers/). This is the simplest way to wait for the container completion. This is also the reason why pipe task resource definition is a ContainerSpec and not a complete Pod specification.
- Main container is a `busybox` image, running the `sleep infinity` command, which purpose is to wait for KUDO to extract and store the files.
...
An alternative approach would use the provided ContainerSpec as a main container (or let the user provide a complete Pod spec). We would inject a sidecar with a go executable which would:
