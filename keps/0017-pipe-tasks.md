...
``` 
The key will be generated by KUDO, avoiding collisions and making sure that it is not used before it is generated. We would generate hierarchical name strings e.g. `deploy.bootstrap.gencert.pipe.certificate-#hash` which would make the belonging of the file clear to the user. However, user references the file by its key `{{.Pipes.Certificate}}`. KUDO will make sure that a key is only set once to avoid accidentally overriding them.
