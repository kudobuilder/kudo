...
then
    ...
    mkdir -p reports/ kind-logs/
    go get github.com/jstemmer/go-junit-report

    go run ./cmd/kubectl-kudo test --config kudo-e2e-test.yaml --artifacts-dir kind-logs 2>&1 |tee /dev/fd/2 |go-junit-report -set-exit-code > reports/kudo_e2e_test_report.xml
